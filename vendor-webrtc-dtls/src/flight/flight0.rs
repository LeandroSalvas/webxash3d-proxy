            ...
            for extension in &client_hello.extensions {
                match extension {
                    Extension::SupportedEllipticCurves(e) => {
                        ...
                        // Pick the first curve we actually implement. Clients (e.g.
                        // Chrome) may list post-quantum curves we don't support
                        // first; blindly taking elliptic_curves[0] then fails the
                        // handshake with ErrInvalidNamedCurve.
                        state.named_curve = e
                            .elliptic_curves
                            .iter()
                            .copied()
                            .find(|curve| *curve != NamedCurve::Unsupported)
                            .unwrap_or(NamedCurve::Unsupported);
                    }
                    Extension::UseSrtp(e) => {
